...
export default function Cart() {
    ...
    useEffect(() => {
        fetch(`https://krispy-kreme-app-beige.vercel.app/api/getData`)
            .then((res) => res.json())
            .then((user) => {
                setUser(user)
            })
    ...
    useEffect(() => {
        if (user) {
            fetch(`https://krispy-kreme-app-beige.vercel.app/api/getCart?user=${user.email}`)
                .then((res) => res.json())
                .then((data) => {
                    setData(data);
                });
    ...
    const handleSubmit = () => {
        ...
        runDBCallAsync(`https://krispy-kreme-app-beige.vercel.app/api/placeOrder?user=${user.email}&products=${JSON.stringify(products)}&dateTime=${new Date()}`)
        runDBCallAsync(`https://krispy-kreme-app-beige.vercel.app/api/removeCart?user=${user.email}`)
        window.location = "/checkout"
    };
    const removeCartItem = (_id) => {
        console.log("ID is: " + _id)
        runDBCallAsync(`https://krispy-kreme-app-beige.vercel.app/api/removeCartItem?_id=${_id}`)
