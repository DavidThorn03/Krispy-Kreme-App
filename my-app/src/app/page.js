...
export default function MyApp() {
  ...
  const handleSubmit = (event) => {
    ...
    if (errorMessage.length > 0) {
      ...
      runDBCallAsync(`https://krispy-kreme-app-beige.vercel.app//api/login?email=${email}&pass=${pass}`)
    }
  };
  ...
  async function runDBCallAsync(url) {
    ...
    if (data.data == "invalid") {
      ...
      await fetch(`https://krispy-kreme-app-beige.vercel.app/api/saveData?email=${data[0].email}&manager=${data[0].manager}`);
      if (data[0].manager) {
        console.log("manager")
